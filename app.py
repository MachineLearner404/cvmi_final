...
app = Flask(__name__)
app.secret_key = 'cvmi-secret-key-2024'
...
USERS = {
    ...
    "alka": {"password": "mentor1", "name": "Dr. Alka Banker", "role": "Mentor"},
    "rahul": {"password": "mentor2", "name": "Dr. Rahul Kumar", "role": "Mentor"},
    "demo": {"password": "demo123", "name": "Demo User", "role": "Researcher"}
}
...
CVMI_STAGE_INFO = {
    "CS1": {"name": "Stage 1 - Initiation","description": "Lower borders of C2, C3, C4 are flat.","age_range": "6-8 years","characteristics": ["Flat lower borders"]},
    "CS2": {"name": "Stage 2 - Acceleration","description": "Concavities in C2 and C3.","age_range": "8-10 years","characteristics": ["C2 & C3 concavities"]},
    "CS3": {"name": "Stage 3 - Transition","description": "Concavities in all vertebrae.","age_range": "10-12 years","characteristics": ["All concavities"]},
    "CS4": {"name": "Stage 4 - Deceleration","description": "Distinct concavities.","age_range": "12-14 years","characteristics": ["Deep concavities"]},
    "CS5": {"name": "Stage 5 - Maturation","description": "More accentuated concavities.","age_range": "14-16 years","characteristics": ["Accentuated concavities"]},
    "CS6": {"name": "Stage 6 - Completion","description": "Growth complete.","age_range": "16-18 years","characteristics": ["Fully developed"]}
}
...
def process_image():
    ...
    # ---------------- SEGMENTATION ---------------- #
    seg_results = seg_model.predict(source=cropped_roi, imgsz=512, verbose=False)
    masks = seg_results[0].masks
    ...
    overlay = np.zeros_like(cropped_roi)
    for i, mask in enumerate(mask_data):
        class_name = names[int(classes[i])]
        color = class_color_map[class_name]

        resized_mask = cv2.resize(mask, (cropped_roi.shape[1], cropped_roi.shape[0]))
        binary = (resized_mask > 0.5).astype(np.uint8)

        for c in range(3):
            overlay[:, :, c] += binary * color[c]

    segmented = cv2.addWeighted(cropped_roi, 1, overlay, 0.5, 0)

    # ---------------- CLASSIFICATION (512 INPUT) ---------------- #
    resized_cls = cv2.resize(cropped_roi, (512, 512))  # ✅ IMPORTANT
    ...
    return jsonify({
        "cvmi_stage": cvmi_stage,
        "confidence": confidence,
        "segmented": numpy_to_base64(cv2.cvtColor(segmented, cv2.COLOR_BGR2RGB))
    })
